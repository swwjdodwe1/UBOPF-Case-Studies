#SCENARIO 1
#Reproduce a situation with excess PV generation and low demand; PV generation is increased compared to the baseline scenario while demand is kept reduced

#Strategy: dynamic limitation of active power at unity power factor
...
P_Demand_a = 0.5*[0, 1500, 0, 0, 0, 900, 1100, 0, 0, 0, 1100, 600, 0, 0, 900, 0, 0, 500]  # Active power demanded on phase a of each node
P_Demand_b = 0.5*[0, 0, 1100, 0, 0, 0, 0, 600, 0, 0, 800, 0, 900, 0, 600, 0, 1000, 700]   # Active power demanded on phase b of each node
P_Demand_c = 0.5*[0, 0, 0, 1000, 0, 0, 0, 0, 900, 0, 1400, 700, 0, 1100, 0, 0, 1000, 0]   # Active power demanded on phase c of each node
Q_Demand_a = 0.5*[0, 130, 0, 0, 0, 50, 40, 0, 0, 0, 100, 90, 0, 0, 50, 0, 0, 90]          # Reactive power demanded on phase a of each node
Q_Demand_b = 0.5*[0, 0, 50, 0, 0, 0, 0, 60, 0, 0, 40, 0, 100, 0, 80, 0, 100, 90]          # Reactive power demanded on phase b of each node
Q_Demand_c = 0.5*[0, 0, 0, 20, 0, 0, 0, 0, 60, 0, 100, 150, 0, 60, 0, 0, 30, 0]           # Reactive power demanded on phase c of each node
...
S_Sol_Gen_ub_a = 1.6*[0, 2500, 0, 0, 0, 2500, 0, 0, 0, 0, 2500, 2500, 0, 0, 0, 0, 0, 2500]        # In phase a
S_Sol_Gen_ub_b = 1.6*[0, 0, 2500, 0, 0, 0, 0, 2500, 0, 0, 2500, 0, 0, 0, 2500, 0, 2500, 2500]     # In phase b
S_Sol_Gen_ub_c = 1.6*[0, 0, 0, 0, 0, 0, 0, 0, 2500, 0, 2500, 2500, 0, 2500, 0, 0, 2500, 0]        # In phase c
...
#Unity power factor operation constraint for photovoltaic generators
@constraint(model, [i in 1:nN], Q_G_solar_a[i]==0)
@constraint(model, [i in 1:nN], Q_G_solar_b[i]==0)
@constraint(model, [i in 1:nN], Q_G_solar_c[i]==0)
